...
const getPatient = (patientId: number): PatientDetails | undefined =>
    retrieve().find((p) => p.patientId === patientId);

const getPatients = (): Patient[] => retrieve();

const retrieve = (): PatientDetails[] =>
    JSON.parse(localStorage.getItem("patients") ?? "[]");

const createPatient = (patient: PatientDetails) => {
    const patientId =
        getPatients().reduce(
            (max, patient) =>
                patient.patientId > max ? patient.patientId : max,
            0
        ) + 1;
