...
def create_items(container):
    ...
    sales_order = get_sales_order("User1", "John", "Smith", "patient")
    container.create_item(body=sales_order)

    sales_order = get_sales_order("User2", "Sherry", "Alexander", "patient")
    container.create_item(body=sales_order)

    sales_order = get_sales_order("User3", "Bryan", "Alexis", "patient")
    container.create_item(body=sales_order)

    sales_order = get_sales_order("User4", "Gregory", "Arnold", "patient")
    container.create_item(body=sales_order)

    sales_order = get_sales_order("User5", "David", "Amstutz", "patient")
    container.create_item(body=sales_order)

    sales_order = get_sales_order("User6", "James", "Thom", "therapist")
    container.create_item(body=sales_order)

    sales_order = get_sales_order("User7", "Sharon", "Anaya", "therapist")
    container.create_item(body=sales_order)

    sales_order = get_sales_order("User8", "Rickey", "Anderson", "therapist")
    container.create_item(body=sales_order)

    sales_order = get_sales_order("User9", "Michael", "Arquette", "therapist")
    container.create_item(body=sales_order)
...
def read_item(container, doc_id, account_number):
    ...
    print('First Name: {0}'.format(response.get('firstname')))
    print('Last Name: {0}'.format(response.get('lastname')))
    print('User Type: {0}'.format(response.get('type')))
...
def get_sales_order(item_id, first_name, last_name, user_type):
    order1 = {'id' : item_id,
                'partitionKey' : 'SystemUsers',
...
def run_sample():
    ...
    try:
        ...
        read_item(container, 'User1', 'SystemUsers')
        read_item(container, 'User2', 'SystemUsers')
        read_item(container, 'User6', 'SystemUsers')
